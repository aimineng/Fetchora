...
- **引擎会自我恢复** —— 运行期间 aria2c 被杀掉的话，几秒内会自动重启，并且"挂了"和"回来了"都会告诉你。它当时在下的任务会交还给新的引擎，由后者从 `.aria2` 控制文件继续续传：下载中途杀掉引擎只会损失几秒，不会丢掉整个队列。
...
`--self-test` 不会开窗口、也不会真的下载，这是刻意的：它要能在没有桌面的机器上跑。
只有真实会话里才会暴露的问题，由 `tools/check-engine-supervision.ps1` 在 CI 的 Windows
任务里对着已构建的程序检查：
...
| 1 | 启动程序会启动引擎。 |
| 2 | 杀掉引擎后程序会拉起一个新的。 |
| 2b | 正在下载的任务会连同已下载的分片交还给新引擎，而不是被判定为已完成。 |
| 2d | 暂停再继续会接着下载，而不是当场"完成"。 |
| 2c | 被移除的任务不会在下一轮轮询里自己回来。 |
| 2e | 同一个链接下载两次会真的存出第二份（`文件名 (2).扩展名`）。 |
| 3 | 强杀程序不会留下孤儿引擎在后台继续下载。 |

传入 `-DownloadUrl` 与 `-SmallDownloadUrl`（用 `tools/testsrv.js` 提供文件）才会跑 2b–2e，
不传就只做进程层面的检查。CI 还会用 `--screenshot` 把每一页渲染成 PNG 并作为产物上传——
绘制期的崩溃是"从不绘制的自检"发现不了的。
